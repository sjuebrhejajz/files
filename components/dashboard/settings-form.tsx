          ...
          <>
            ...
            <div className="flex flex-col gap-8">
              ...
              {category === "perks" && (
                <>
                  <MusicSection />
                  {user.role === "admin" && <VideoBackgroundSection />}
                  <ThemeSection />
                </>
              )}
...
// ---------------- profile video background (admin-only) ----------------
...
function VideoBackgroundSection() {
  ...
  const onFile = async (file: File) => {
    ...
    setLoading(true)
    try {
      const { uploadUrl, key } = await call("/api/user/settings/video-url", {
        contentType: file.type,
        size: file.size,
      })
      const put = await fetch(uploadUrl, { method: "PUT", body: file, headers: { "Content-Type": file.type } })
      if (!put.ok) throw new Error("Upload failed.")
      await call("/api/user/settings/video", { key })
      setStatus((s) => (s ? { ...s, url: `/a/${key}` } : s))
    } catch (err) {
      setError((err as Error).message)
    } finally {
  ...
  const toggle = async () => {
    ...
    setError(null)
    try {
      await call("/api/user/settings/video", { enabled: next })
      setStatus((s) => (s ? { ...s, enabled: next } : s))
    } catch (err) {
      setError((err as Error).message)
    } finally {
  ...
  const remove = async () => {
    ...
    setError(null)
    try {
      await call("/api/user/settings/video", { remove: true })
  ...
  return (
    <Section title="Profile background video (admin)">
      <Msg error={error} info={null} />
      <p className="mb-2 text-xs text-muted-foreground">
        Upload a video to play as your public profile&apos;s full-page background. It always plays muted, no matter
        what — use the music widget above for actual sound.
      </p>
      {status.url ? (
        <video src={status.url} muted loop controls className="mb-3 max-h-40 w-full rounded-md object-cover" />
      ) : (
        <p className="mb-3 text-xs text-muted-foreground">No video uploaded yet.</p>
      )}
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative inline-flex">
          <span
            ...
            className="pointer-events-none flex items-center rounded-md bg-secondary px-3 py-1.5 text-xs font-medium text-secondary-foreground"
          >
            {loading ? "Working…" : status.url ? "Replace video" : "Upload video"}
          </span>
          <input
            type="file"
